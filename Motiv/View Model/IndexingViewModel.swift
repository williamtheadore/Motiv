        ...
        DispatchQueue.main.async {
            self.userIndex.search(query: query) { serverResponse in
                ...
                case .success(let result):
                    ...
                    for hit in hits! {
                        users.append(User(id: hit.id ?? "", name: hit.name ?? "", username: hit.username ?? "", program: hit.program ?? "", school: hit.school ?? "", friends: hit.friends ?? [], requests: hit.requests ?? [], houseUID: "", profilePhoto: hit.profilePhoto ?? ""))
                    }
